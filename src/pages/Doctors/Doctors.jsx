...
import { getDoctorStatusClass } from '../../utils/StatusColor';
import {doctorsCardData ,  doctorsData} from "../../data/Data"
...
const Doctors = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="flex flex-col min-[450px]:flex-row min-[450px]:items-center min-[450px]:justify-between gap-5">
        <div>
        ...
        <button className="w-full min-[450px]:w-auto flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl shadow-md hover:shadow-lg transition">
          <Plus size={18} />
      ...
      <div className="grid grid-cols-1 min-[468px]:grid-cols-2 min-[768px]:grid-cols-3 min-[1280px]:grid-cols-4 gap-6">
        {doctorsCardData.map((card, index) => (
          <div key={index} className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md flex flex-col gap-6 hover:scale-105 transition">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md py-6 px-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5 py-2">
                ...
                {departmentOpen && (
                  <div className="absolute mt-2 w-full sm:w-52 bg-white dark:bg-slate-900 rounded-lg shadow-xl border dark:border-slate-700 z-50 dark:text-white">
                    {departments.map((item) => (
                      <div
                        key={item}
                        onClick={() => { setDepartment(item); setDepartmentOpen(false); }}
          ...
          {/* Filter Modal */}
          {filterOpen && (
            <div className="fixed inset-0 bg-black/60 dark:bg-black/80 z-[100] flex items-end">
              <div className="w-full bg-white dark:bg-slate-900 rounded-t-3xl p-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button 
                    ...
                    className="p-3 rounded-full hover:bg-gray-100 dark:hover:bg-slate-700 transition"
                  >
                    <X size={26} className="text-gray-500 dark:text-gray-400" />
                  </button>
                </div>
                ...
                <div className="flex gap-3">
                  <button 
                    onClick={() => setFilterOpen(false)}
                    className="flex-1 border border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white py-3.5 rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-slate-700"
                  >
                    Reset
                  </button>
                  <button 
        ...
        <div className="overflow-x-auto hide-scrollbar">
          <table className="w-full sm:min-w-[850px]">
            <thead>
              <tr>
                <th className="px-4 py-3 text-center dark:text-white sticky left-0 z-20 rounded-xl bg-white dark:bg-slate-800">Doctor</th>
                <th className="px-4 py-3 text-center dark:text-white">Speciality</th>
                <th className="px-4 py-3 text-center dark:text-white">Department</th>
                <th className="px-4 py-3 text-center dark:text-white">Experience</th>
            ...
            <tbody>
              {doctorTable.map((item, index) => (
                <tr key={item.id} className={`${index !== doctorTable.length - 1 ? "border-b border-slate-200 dark:border-slate-700" : ""} hover:bg-slate-50 dark:hover:bg-slate-700 transition`}>
                  ...
                  <td className="px-4 py-4 text-center dark:text-white whitespace-nowrap">{item.specialty}</td>
                  <td className="px-4 py-4 text-center dark:text-white whitespace-nowrap">{item.department}</td>
                  <td className="px-4 py-4 text-center dark:text-white whitespace-nowrap">{item.experience}</td>
                  ...
                  <td className="px-4 py-4 text-center dark:text-white">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getDoctorStatusClass(item.status)}`}>
                      {item.status}
                    </span>
                  </td>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 px-4 border-t border-slate-200 dark:border-slate-700">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="w-9 h-9 rounded-lg border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white transition">4</button>
            <span className="px-2 text-slate-500 dark:text-slate-400">...</span>
            <button className="w-9 h-9 rounded-lg border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white transition">10</button>
            <button className="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
